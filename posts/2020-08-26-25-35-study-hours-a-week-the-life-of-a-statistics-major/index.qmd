---
title: "Study 25-35 Hours a Week? The Life of a First-Year Statistics Major"
author: "William Medwid"
date: '2020-08-26'
categories: [analysis, academics, r]
image: "image.png"
format:
  html:
    code-fold: true
...
widget <- DT::datatable(study_time %>% select(Completed,Class,Assignment,Minutes) %>% head(100), rownames = FALSE)

widgetframe::frameWidget(widget, width='100%', height = '100%')
```

Next, we'll aggregate these tasks and see how much work there was on a weekly basis. For this plot, each week is its own dot, with a smoothed trendline interpolating a moving average. 
...
So, does Cal Poly require a full 25-35 hours a week of homework? In my first year I haven't needed quite so much - just `r round(conf_interval$estimate, 1)` hours per week outside of class, but it's still felt rigorous and very effective in teaching a lot. Longer hours may be coming in the following years.
